...
export default function ModifyStaffProfile() {

  const { role } = useContext(AuthContext);
  const { profileDetails, updateProfileDetails, availableRoles, availableServices, checkRoleIsServiceProvider, clearServiceProvided, } = useContext(StaffProfileContext);
  const { loading, error, handleSubmitForEditStaff, fetchRoles, fetchServices, } = useStaffProfileConfiguration();
  ...
  return (
    <div>
      <form className="my-4 container mx-auto w-5/6 bg-gray-50 rounded-lg shadow-md shadow-gray-300 flex flex-wrap md:items-end gap-8 px-12 py-12 mx-auto" onSubmit={(e) => handleSubmitForEditStaff(e)}>
        <div className="relative w-full">
          <div className="flex justify-center items-center mb-4 h-16">
            <span className="font-bold text-xl text-gray-900">Edit Staff Profile</span>
          </div>
        ...
        <Dropdown
          ...
          loadSelection={profileDetails.staffRole}
          options={[
            { label: '', value: '' },
        ...
        {profileDetails.staffRole !== null && isServiceProvider && < Checkbox
          ...
          selectedServices={profileDetails.servicesProvided}
          handleOnChange={updateProfileDetails}
        ...
        <div className="relative md:w-2/5 w-full h-auto mx-auto">
          <textarea
            ...
            className="peer w-full bg-transparent text-gray-900 font-normal outline outline-0 focus:outline-0 disabled:bg-white disabled:border-0 transition-all placeholder-shown:border placeholder-shown:border-gray-100 placeholder-shown:border-t-gray-100 border focus:border-2 border-t-transparent focus:border-t-transparent text-sm px-3 py-2.5 rounded-[7px] border-gray-100 focus:border-gray-900 resize-none" // Added 'resize-none' to disable textarea resizing
            placeholder=""
            onChange={(e) => updateProfileDetails(e)}
            pattern="^[\s\S]{1,255}$"
